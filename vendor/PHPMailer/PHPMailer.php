            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (preg_match('#^data:(image/(?:jpe?g|gif|png));?(base64)?,(.+)#', $url, $match)) {
                    if (count($match) == 4 and static::ENCODING_BASE64 == $match[2]) {
                        $dataView = base64_decode($match[3]);
                    } elseif ('' == $match[2]) {
                        $dataView = rawurldecode($match[3]);
                    } else {
                    ...
                    $cid = hash('sha256', $dataView) . '@phpmailer.0'; // RFC2392 S 2

                    if (!$this->cidExists($cid)) {
                        $this->addStringEmbeddedImage($dataView, $cid, 'embed' . $imgindex, static::ENCODING_BASE64, $match[1]);
                    }
                    $message = str_replace(
                        $images[0][$imgindex],
